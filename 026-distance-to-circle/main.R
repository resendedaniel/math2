...
circle <- function(angle, band=1.3) {
    x <- band ^ angle
    y0 <- sqrt(band^2 - (x)^2 + 1)
    y1 <- sqrt((1/band)^2 - (x)^2 + 1)
...
ggplot(circle_df, aes(x, y)) +
    ...
    scale_x_continuous(breaks = round(as.numeric(unlist(options()$trades_threshold)), 1)) +
    scale_y_continuous(breaks = round(as.numeric(unlist(options()$trades_threshold)), 1))
    # scale_y_log10() +
    # scale_x_log10()
